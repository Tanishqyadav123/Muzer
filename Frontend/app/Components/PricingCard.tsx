  ...
  return (
    <div className="w-[20vw] min-h-[50vh] bg-[#1f2a38] p-4 rounded-lg">
      <div className="flex justify-between items-center h-[10%]">
        <p className="package-tier rounded-full flex items-center justify-center uppercase bg-[#374152]  min-w-[35%]">
          {tier}
        </p>

        <p className="package-pricing text-gray-500 text-sm">
          ${pricing}/{"month"}
        </p>
      </div>
      ...
      <div className="package-points mt-2 flex items-start justify-center flex-col">
        {features.map(
          (elem: { title: string; checked: boolean }, index: number) => {
            return (
              <div
                key={index}
                className={`flex items-center gap-2 my-3 text-sm ${
                  elem.checked ? "text-white" : "text-[#5e6066]"
                }`}
              >
      ...
      <div className="mt-10 flex items-center justify-center">
        <PricingCardButton
          btnText={btnText}
          color={tier === "Premium" ? "bg-gray-400" : "bg-[#1f2a38]"}
        />
      </div>
    </div>
  );
