...
): Promise<{ success: boolean; errMessage?: string }> => {
...
export const sendOtpOnEmailService = async (
  sendEmail: VerifyEmailSchemaType
): Promise<boolean> => {
  try {
    const res: any = await axios.post(
...
export const initiateOAuthService = async () => {
  try {
